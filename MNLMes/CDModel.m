...
//    DLog(@"%@",[[NSFileManager defaultManager] applicationSupportDirectory]);
    ...
    NSURL *storeURL = [libraryPath URLByAppendingPathComponent:@"MES.sqlite"];
...
-(Nodes*)addNewNodeWithX:(double)x 
                       Y:(double)y 
                      DX:(double)dx 
                      DY:(double)dy{
    ...
    nod.x = [NSNumber numberWithDouble:x];
    nod.y = [NSNumber numberWithDouble:y];
    nod.dx = [NSNumber numberWithDouble:dx];
    nod.dy = [NSNumber numberWithDouble:dy];
...
-(void) removeElementWithNodeWithNumber:(NSInteger)number{
    ...
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"Elements" inManagedObjectContext:myManagedObjectContext];
    NSFetchRequest *request = [[NSFetchRequest alloc] init]; 
    [request setEntity:entity]; 
    Nodes* n = [self getNodeWithNumber:number];
    NSPredicate *myNodeQuestion = [NSPredicate predicateWithFormat:@"n1 = %@ || n2 = %@ || n3 = %@", n, n, n];
    [request setPredicate:myNodeQuestion];
    NSError *error = nil;  
    NSMutableArray *mutableFetchResults = [[myManagedObjectContext executeFetchRequest:request error:&error] mutableCopy];
    
    if ([mutableFetchResults count] > 0) {
        for (Elements* del in mutableFetchResults) {
            [myManagedObjectContext deleteObject:del];
            [myManagedObjectContext save:&error];
            if (error != nil) {
                //TODO: error hendeling
